    ...
    with open("目标大学.csv", "r", encoding="utf-8") as csvfile:
        ...
        for student in students:
            try:
                ...
                content = (
                    template.replace(r"{student_name}", student["姓名"])
                    .replace(r"{university_name}", student["目标大学"])
                    .replace(r"{issue_date}", issue_date)
                    .replace(r"{stamp_image}", f"icons/{student['目标大学']}.png")
                    .replace(
                        r"{university_logo}", get_university_logo(student["目标大学"])
                    )
                    .replace(
                        r"{background}", student.get("background", "images/blue_bg.jpg")
                    )
                ...
                print(
                    f"生成{student.get('姓名', '未知')}的通知书时出错：[{type(e).__name__}] {str(e)}\n数据记录：{dict(student)}"
                )
